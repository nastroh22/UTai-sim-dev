...
def run_on_thread(sample_size, belief_factor, assets, risk_params, cdp_rate, tx_fee, run_index, eth_price_per_day, days_per_config,
    ...
    for day in range(0, days_per_config):
        # NOTE : Right here every user is being given an identical Rho -- but this could also be informed by Real-World Data
...
def run_tests(sample_size, belief_factor, cdp_rates, tx_fees, runs, eth_price_per_day, days_per_config, test_type, logdir, logger,
    ...
    print(np.array(assets_runs[0]).shape)

    for tx_fee in tx_fees:
        for cdp_rate in cdp_rates:
            for run in range(runs):
                # NOTE: the bug downstream is again caused by belier_factor
                args.append(
    ...
    # Get all required parameters and dump to pickle #NOTE (3,4,5)->(4,5,6)
    blf_axis = [args[i][2] for i in range(len(args))] # NOTE -- if we want to use varying belief/ Add here
    cdp_axis = [args[i][4] for i in range(len(args))]
    txf_axis = [args[i][5] for i in range(len(args))]
    run_axis = [args[i][6] for i in range(len(args))]
