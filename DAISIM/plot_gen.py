...
# NOTE : ** be aware these are hard-coded values **  ( caused a bug)
hi_risk_inv = 0
lo_risk_inv = 2

#NOTE: See sample.config, in that case the lo-risk and hi-risk are --> 
# hi_risk_inv = 6
# lo_risk_inv = 9
...
def generate_asset_curve(cdp_axis, txf_axis, dai_price_history_axis, asset_history_axis, cur_dir, run_id):
    ...
    #TODO  Check this before each run! Might need to change this before each run.
    eth_price = 130

    cdp_rates = set()
    tx_fees = set()
    print(asset_history_axis)
...
def gen_plots_for_run(filename, data, run_id):
    ...
    # print("here: " , asset_history_filter)
    # print("Depends on: ", range(len(run_axis)))
